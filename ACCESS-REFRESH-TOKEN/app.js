...
function createAccessToken(id) {
  const accessToken = jwt.sign(
    { id: id }, // JWT 데이터, id라는 키에 id로 받아온 값을 넣어준다.
    SECRET_KEY, // 비밀키
    { expiresIn: "10s" }
...
function createRefreshToken() {
  const refreshToken = jwt.sign(
    {}, // JWT 데이터, id에 대한 데이터가 담겨있지않아서 오래 가지고 있어도 됨 
    SECRET_KEY, // 비밀키
    { expiresIn: "7d" }
...
app.get("/get-token", (req, res) => {
  ...
  if (!isAccessTokenValidate) {
    ...
    if (!accessTokenId)
      return res
        .status(419) //토큰은 정상적이나 서버가 가지고있는 토큰과 일치하지않을때, 탈취당했을때 해당하는 토큰 자체를 만료시킬때 발생
